...
export default function MenuItem({ iconString, colorString, sizeString }: MenuItemTypes) {
  ...
  const icons = () => {
    if (iconString == '关于你') return <AiOutlineHome size={sizeString} color={colorString} />
    if (iconString == '关注') return <RiGroupLine size={sizeString} color={colorString} />
    if (iconString == '直播') return <BsCameraVideo size={sizeString} color={colorString} />
  }

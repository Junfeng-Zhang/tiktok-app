...
export default function CommentsHeader({ post, params }: CommentsHeaderCompTypes) {
  ...
  const hasUserLikedPost = () => {
    ...
    setUserLiked(res ? true : false); // 避免 useIsLiked 成为异步函数
  }
  ...
  const likeOrUnlike = () => {
    if (!contextUser?.user) return setIsLoginOpen(true); // 检测用户是否登录

    let res = useIsLiked(contextUser.user.id, params.postId, likesByPost)
    if (!res) {
      like()
    } else {
      likesByPost.forEach(like => { // 查看点赞
  ...
  const deletePost = async () => {
    let res = confirm('确定删除此作品吗?')
    if (!res) return

    setIsDeleteing(true)

    try {
      await useDeletePostById(params?.postId, post?.video_url); // 一旦deletePost成功，就重定向回发布帖子的个人资料
